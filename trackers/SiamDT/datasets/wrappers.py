...
# Kaggle Anti-UAV410 dataset
ANTI_UAV410_ROOT = "/kaggle/input/datasets/uppalajathin/anti-uav-410-sample"
...
def _datasets(name):
    ...
    if name == "coco_train":
        ...
        return data.GOT10k(
            root_dir="/media/data2/TrackingDatasets/GOT-10k/full_data/train_data",
            subset="train",
        )
        ...
        return data.LaSOT(
            root_dir="/media/data2/TrackingDatasets/LaSOT/", subset="train"
        )

    elif name == "uavtir_train":
        return data.UAVtir(root_dir=ANTI_UAV410_ROOT, subset="train")

    elif name == "uavtir_val":
        return data.UAVtir(root_dir=ANTI_UAV410_ROOT, subset="val")
...
def _transforms(name):
    ...
    if name == "basic_train":
        ...
        return data.ExtraPairTransforms(
            with_photometric=True, with_expand=False, with_crop=False
        )
...
class PairWrapper(data.PairDataset):
    def __init__(
        ...
        **kwargs,
        ...
        self.indices = self._setup_indices(self.base_dataset, max_size)
    ...
    def __getitem__(self, index):
        ...
        if index == 0:
            self.indices = self._setup_indices(self.base_dataset, self.max_size)
        ...
        if len(item["gt_bboxes_z"]) == 0 or len(item["gt_bboxes_x"]) == 0:
            return self._random_next()

        # sample up to "max_instances" instances
        if self.max_instances > 0 and len(item["gt_bboxes_z"]) > self.max_instances:
            indices = np.random.choice(
                len(item["gt_bboxes_z"]), self.max_instances, replace=False
            )
        ...
        # attach class labels if required
        if self.with_label:
            _tmp = item["gt_bboxes_x"].data
            item["gt_labels"] = DC(_tmp.new_zeros(len(_tmp)).long())
    ...
    def _setup_indices(self, base_dataset, max_size):
        ...
        if max_size > 0 and len(base_dataset) > max_size:
            indices = np.random.choice(len(base_dataset), max_size, replace=False)
        else:
            indices = np.arange(len(base_dataset))

        return indices

    def _setup_base_dataset(
        self, base_dataset, base_transforms, sampling_prob, max_size
    ):
        ...
        for name in names:
            if "coco" in name:
                # image-style dataset
                dataset = data.Image2Pair(_datasets(name), _transforms(base_transforms))
            else:
                # sequence-style dataset
                dataset = data.Seq2Pair(_datasets(name), _transforms(base_transforms))
