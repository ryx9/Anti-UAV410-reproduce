...
def main():
    ...
    checkpoint_file = "/kaggle/input/models/rizwanabdullah05/siamdt-models/pytorch/default/1/siamdt_swin_tiny_sgd.pth"
    ...
    anno_files = sorted(glob.glob(os.path.join(videofilepath, "*/IR_label.json")))
    ...
    for seq_num, seq_name in enumerate(seq_names, start=1):
        ...
        img_files = sorted(
            glob.glob(os.path.join(videofilepath, seq_name + "/img/*.jpg"))
        )
        ...
        for index, image_file in enumerate(img_files, start=1):
            if index == 1:
                ...
                if vis_flag:
                    ...
                    cv2.namedWindow(
                        display_name, cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO
                    )
            ...
            # elif index<10:
                ...
                if vis_flag:
                    ...
                    if len(output_box) > 0:
                        cv2.rectangle(
                            frame,
                            (int(output_box[0]), int(output_box[1])),
                            (
                                int(output_box[2] + output_box[0]),
                                int(output_box[3] + output_box[1]),
                            ),
                            (0, 255, 0),
                            5,
                        )
        ...
        print(
            "%d/%d: %20s %5s Fixed Measure: %.03f"
            % (seq_num, len(seq_names), seq_name, mode, mixed_measure)
        )
