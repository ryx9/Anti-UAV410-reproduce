...
class Tracker:
    ...
    def __init__(
        ...
        tracker_module_abspath = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "..", "tracker", self.name)
        )
    ...
    def run_sequence(self, seq, visualization=None, debug=None, multiobj_mode=None):
        ...
        if multiobj_mode == "default" or is_single_object:
            ...
            tracker = MultiObjectWrapper(self.tracker_class, params, None)
        else:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        output["object_presence_score_threshold"] = tracker.params.get(
            "object_presence_score_threshold", 0.55
        )

        return output

    def run_video_generic(
        self, debug=None, videofilepath=None, optional_box=None, save_results=False
    ):
        ...
        if multiobj_mode == "default":
            ...
            tracker = MultiObjectWrapper(
                self.tracker_class, params, None, fast_load=True
            )
        else:
            raise ValueError("Unknown multi object mode {}".format(multiobj_mode))
        ...
        while True:
            ...
            if ui_control.mode == "select":
                cv.rectangle(
                    frame_disp, ui_control.get_tl(), ui_control.get_br(), (255, 0, 0), 2
                )
            ...
            if len(sequence_object_ids) > 0:
                ...
                if "segmentation" in out:
                    ...
                    if save_results:
                        ...
                        cv.imwrite(
                            self.results_dir + f"seg_{frame_number}.jpg", mask_image
                        )
    ...
    def run_vot2020(self, debug=None):
        ...
        if output_segmentation:
            ...
            bbox = (
                masks_to_bboxes(torch.from_numpy(vot_anno_mask), fmt="t")
                .squeeze()
                .tolist()
            )
        else:
